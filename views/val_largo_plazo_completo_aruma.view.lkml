...
view: val_largo_plazo_completo_aruma {
  ...
  dimension: Periodomes {
    type: string
      sql:Case when extract(month from val_largo_plazo_completo_aruma.fecha) = 1  then 'Ene'
               ...
               when extract(month from val_largo_plazo_completo_aruma.fecha) = 12 then 'Dic'
               else 'Error'
    end
    ;;
    }
